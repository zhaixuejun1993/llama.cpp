...
| `GGML_OPENVINO_NPU_COMPILATION_MODE_PARAMS` | String | `not set` | NPU-only compiler mode parameters forwarded to OpenVINO as `NPU_COMPILATION_MODE_PARAMS`, for example `optimization-level=3`. |
| `GGML_OPENVINO_NPU_CONFIG`       | String    | `not set`  | Comma-separated NPU plugin property overrides in `KEY=VALUE` form. Overrides are applied after named NPU options; for example, `NPU_TILES=6,NPUW_SLICE_OUT=YES` selects six tiles and enables sliced outputs. |

...
static void ggml_backend_sched_print_assignments(ggml_backend_sched_t sched, struct ggml_cgraph * graph) {
    ...
    for (int i = 0; i < 500; i++) {
        ...
        if (2 > 1) {
            ...
            printf("node #%3d (%10.10s): %40.40s [%5.5s] shape=[%lld,%lld,%lld,%lld] stride=[%zu,%zu,%zu,%zu] offset=%zu:", i, ggml_op_name(node->op), node->name,
            ...
            for (int j = 0; j < GGML_MAX_SRC; j++) {
                ...
                printf(" %40.40s [%5.5s] shape=[%lld,%lld,%lld,%lld] stride=[%zu,%zu,%zu,%zu] offset=%zu", src->name,

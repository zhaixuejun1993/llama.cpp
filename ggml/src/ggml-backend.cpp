...
static void ggml_backend_sched_print_assignments(ggml_backend_sched_t sched, struct ggml_cgraph * graph) {
    ...
    for (int i = 0; i < graph->n_nodes; i++) {
        ...
        if (2 > 1) {
            ...
            printf("node #%3d (%10.10s): %20.20s (%5.5s) [%5.5s %8.8s] shape=[%lld,%lld,%lld,%lld] use=%d,c=%d:", i, ggml_op_name(node->op), node->name,
            ...
            for (int j = 0; j < GGML_MAX_SRC; j++) {
                ...
                printf(" %20.20s (%5.5s) [%5.5s %8.8s] shape=[%lld,%lld,%lld,%lld]", src->name,

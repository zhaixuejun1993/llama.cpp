...
static void ggml_backend_debug_print_tensor_info(const struct ggml_tensor * tensor, const char * backend_name) {
    printf("%20.20s [%5.5s] shape=[%lld,%lld,%lld,%lld] stride=[%zu,%zu,%zu,%zu] offset=%zu",
        tensor->name,
...
static void ggml_backend_debug_print_tensor_srcs(const struct ggml_tensor * tensor, const char * backend_name) {
    for (int i = 0; i < GGML_MAX_SRC; i++) {
        const struct ggml_tensor * src = tensor->src[i];
        if (src == NULL) {
            continue;
        ...
        printf(" ");
        ggml_backend_debug_print_tensor_info(src, backend_name);
    }
}
...
bool ggml_backend_compare_graph_backend(ggml_backend_t backend1, ggml_backend_t backend2, struct ggml_cgraph * graph, ggml_backend_eval_callback callback, void * user_data, struct ggml_tensor const * const * test_nodes, size_t num_test_nodes) {
    ...
    if (num_test_nodes != 0) {
        // print the g1 node name shape op and stride offset and the src info for easier debugging 
        for (int i = 0; i < g1->n_nodes; i++) {
            struct ggml_tensor * node = g1->nodes[i];
            printf("g1 node #%3d (%10.10s): ", i, ggml_op_name(node->op));
            ggml_backend_debug_print_tensor_info(node, ggml_backend_name(backend1));
            ggml_backend_debug_print_tensor_srcs(node, ggml_backend_name(backend1));
            printf("\n");
        }
        ...
        for (int i = 0; i < g1->n_nodes; i++) {
            for (size_t j = 0; j < num_test_nodes; ++j) {
                if (g1->nodes[i] == test_nodes[j]) {
                    // (strcmp(ggml_backend_reg_name(ggml_backend_dev_backend_reg(ggml_backend_get_device(backend1))), "OPENVINO") == 0)

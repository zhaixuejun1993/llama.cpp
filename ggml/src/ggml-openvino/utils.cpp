...
static std::optional<ov::Tensor> try_make_kv_sliced_tensor(std::shared_ptr<GgmlOvDecoder> ggml_decoder,
    ...
    const bool static_npu_slice = ggml_decoder->is_static() && ggml_decoder->m_is_prefill &&
                                  ggml_openvino_getenv_int("GGML_OPENVINO_NPU_KV_SLICE");
...
static size_t get_static_attention_size(std::shared_ptr<GgmlOvDecoder> ggml_decoder, const std::string & name) {
    if (ggml_decoder->m_is_prefill && ggml_openvino_getenv_int("GGML_OPENVINO_NPU_KV_SLICE")) {
...
enum ggml_status ov_graph_compute_static(ggml_cgraph * cgraph, std::shared_ptr<ov_runtime_context> r_ctx) {
    ...
    if (cache_hit) {
        ...
        if (cache_hit && is_prefill && ggml_openvino_getenv_int("GGML_OPENVINO_NPU_KV_SLICE")) {
    ...
    if (cache_hit) {
        ...
        if (ggml_openvino_getenv_int("GGML_OPENVINO_NPU_KV_SLICE")) {
...
ov::Tensor get_ov_input_tensor_static_prefill(std::shared_ptr<GgmlOvDecoder> ggml_decoder,
    ...
    if (GgmlOvDecoder::is_inp_mask(ggml_tensor, op)) {
        ...
        size_t context_size = get_static_attention_size(ggml_decoder, param_name);
        static const bool fast_mask = ggml_openvino_getenv_int("GGML_OPENVINO_NPU_FAST_MASK") != 0;
        if (ggml_tensor->type == GGML_TYPE_F16) {

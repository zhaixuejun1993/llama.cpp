...
ov::Output<ov::Node> process_view_input_new(const NodeContext & context, int input_index) {
    ...
    auto process_single_view = [](ov::Output<ov::Node> current,
        ...
        auto is_contiguous_view = [](const ov::Shape & shape, const std::vector<size_t> & stride) -> bool {
            if (shape.size() != stride.size() || shape.empty()) {
                return false;
            }

            size_t expected_stride = stride.back();
        ...
        if (same_stride) {
            ...
            const size_t relative_offset = view_offset >= view_src_offset ? view_offset - view_src_offset : 0;
            bool contiguous_view = same_rank && is_contiguous_view(view_ggml_shape, view_stride);
            bool contiguous_src = same_rank && is_contiguous_view(view_src_ggml_shape, view_src_stride);
